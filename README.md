...
* `卒業論文.pdf`: 製本して提出した最終版の卒業論文
* `背表紙.pdf`: 製本時に使用した背表紙のファイル
* `参考文献_詳細.pdf`: 参考文献の内容を簡潔にまとめた資料

### 02_Presentation/
学会や発表会で使用した資料群です．
* `自律分散シンポジウム/`: 発表スライド（PDF）と前刷り原稿（PDF）
* `卒論発表会/`: 発表スライド（PDF）と前刷り原稿（PDF）

### 03_Data/
研究で使用した解析データや検証データのメインフォルダです．
...
* **姿勢推定CSV(ViTPose)/**
  * ViTPoseによる姿勢推定結果．**今後の各種解析の基盤となる必須データ．**
* **CSV/**
  * `ROMBERG/`: ロンベルグ試験関連．重心の分散データ（`ROMBERG_AllVariance_summary.csv`を使用）と，軌跡長・凸包面積のプロット・CSV（フィルター「あり」を使用）
  * `CoG/`: 2次元の重心位置（CSV）．setting2の静止姿勢を前提とした算出法のため，ロンベルグ試験専用．（被験者C1については左半身のデータのみから算出）
* **モーキャプによる精度検証.zip**
  * モーションキャプチャとViTPoseの精度比較用データ．
  * `detected`: ViTPose解析後の2次元座標CSV
  * `ViT`: detectedを3次元復元した3次元CSV
  * `HUMAN`: モーキャプマーカーを付けた人体の動画
  * `mocap`: モーキャプで取得した3次元関節座標のCSV（全関節検出時のみ切り出しのため長さはバラバラ）
  * `static point`: アルコマーカーを用いた静止点比較用動画（失敗のため未使用）
* **ステレオキャリブレーション.zip**
  * `camera_params_stereo.npz`（3次元復元用パラメータ）とトリミング後動画．
  * `CIPN`, `NOCIPN`は被験者ごとにフォルダ分け．
  * `calib`には`STUDENT_WALK(setting1のみ)`が日付・被験者番号付きで格納．
  * `その他`には精度検証時のキャリブレーション情報（「20260108」を使用）．

### 04_Protocol/
* `実験プロトコル`: 実験内容がまとまった資料．実験時に印刷して参照するためのもの．

## ⚠️ 解析における重要な注意事項
* **基準データ**: 各種解析は `03_Data/姿勢推定CSV(ViTPose)/` のデータをもとに行うこと．
* **歩行タスク (setting1)**: 3次元の重心座標には `CoM_3D` のCSVを使用すること．（※卒論本編では未使用）
* **ロンベルグ試験**: 2次元重心位置は `CoG` のCSVを参照．静止姿勢を前提とした算出法のため，ロンベルグ試験のみに適用すること．（被験者C1については左半身のデータのみから算出）
* **実験プロトコル**: 休憩時間はすべて**30秒**で統一しています．
</details>
