...
VARIANCE_SUMMARY_ROOT = os.path.join(BASE_ROOT, r"data/3_summary_feature/ROMBERG_Variance/summary/ROMBERG_AllVariance_summary.csv")


# 結果出力先

OUTPUT_DIR = os.path.join(BASE_ROOT, r"daily_results/20260216/Crawford_Variance_RedBlue_WithGroupStats_filtered")
...
if df_all is None:
    ...
    data = {
        'subject': ['Sub1', 'Sub2', 'Sub3', 'Ctrl1', 'Ctrl2', 'Ctrl3', 'Ctrl4', 'Ctrl5', 'Ctrl6', 'Ctrl7', 'Ctrl8', 'Ctrl9', 'Ctrl10'],
        'Group': ['CIPN', 'NOCIPN', 'CIPN'] + ['STUDENT']*10,
        'camera': ['C1']*13,
        'Local_EO': np.random.rand(13)*10, 'Local_EC': np.random.rand(13)*15, 'Local_RR': np.random.rand(13)*2,
        'Global_EO': np.random.rand(13)*20, 'Global_EC': np.random.rand(13)*25, 'Global_RR': np.random.rand(13)*2,
        'Total_EO': np.random.rand(13)*30, 'Total_EC': np.random.rand(13)*35, 'Total_RR': np.random.rand(13)*2
    }
...
# ▼▼▼ フォントサイズ設定 (参考コードに合わせる) ▼▼▼
plt.rcParams['font.family'] = 'Arial' # 日本語フォントがあれば 'MS Gothic' などに変更
...
sns.set_theme(style="whitegrid", rc={
    "axes.edgecolor": ".8", 
    "grid.color": ".9",
    "font.family": "Arial", # 日本語フォントがあれば変更
    "font.size": 30,
    "axes.labelsize": 30,
    "xtick.labelsize": 30,
    "ytick.labelsize": 30
...
for cam in CAMERAS:
    ...
    for cat_name, metrics in plot_categories.items():
        # 文字が大きいので図のサイズも大きくする (参考コードに合わせて調整)
